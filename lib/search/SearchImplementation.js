...
import { setIntersection, setUnion } from "@/lib/search/SetUtils";
...
export default class Search {
    ...
    search(searchTokens) {
        ...
        const realTokens = searchTokens.filter(Boolean);
        const completion = this.getRecommendedSearchCompletion(searchTokens);
        const results = this.getSearchResults(realTokens);
        const matchedWords = null; // this.getMatchedWords(realTokens); // FIXME: DO NOT COMMIT TO CODE REPOSITORY!
        return { results, completion, matchedWords };
    }

    /**
     *
     * @param searchTokens
     * @returns { id: Set of words }
     */
    getMatchedWords(searchTokens) {
        const matchedWords = searchTokens.reduce((matchedWords, token) => {
            const mv = this.trie.getMatchedValues(token);
            Object.entries(mv).forEach(([id, words]) => {
                if (id in matchedWords) {
                    matchedWords[id] = setUnion(matchedWords[id], words);
                } else {
                    matchedWords[id] = words;
                }
            });
            return matchedWords;
        }, {});
