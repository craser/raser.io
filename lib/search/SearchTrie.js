export default class SearchTrie {
    ...
    getMatchedValues(key) {
        let node = key ? this.getNode(key) : this;
        const matchedValues = {}; // key --> values
        if (node) {
            if (node.values.size) {
                matchedValues[''] = node.values;
            }

            for (const child in node.children) {
                const c = node.children[child];
                const mv = c.getMatchedValues();
                const prefix = `${key ? key : ''}${child}`;
                Object.entries(mv).forEach(([suffix, values]) => {
                    matchedValues[`${prefix}${suffix}`] = values;
                });
            }
        }
